...
async function handleLine(line: string): Promise<void> {
  if (!line.trim()) return;
  const request = JSON.parse(line) as { id?: Json; method?: string; params?: Record<string, unknown> };
  try {
...
async function handleRequest(method: string, params: Record<string, unknown>): Promise<Json> {
...
function asRoomChat(args: Record<string, unknown>): { roomId: string; chatId: string } {
...
function roomChatSchema(): Json {
  return objectSchema(roomChatProps(), ["roomId", "chatId"]);
}

function roomChatProps(): Record<string, Json> {
  return {
    roomId: { type: "string" },
    chatId: { type: "string" },
    cwd: { type: "string" }
...
function objectSchema(properties: Record<string, Json>, required: string[]): Json {
