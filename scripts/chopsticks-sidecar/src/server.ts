...
async function routeApi(pathname: string, request: IncomingMessage, response: ServerResponse): Promise<void> {
...
async function callApi(pathname: string, body: Record<string, unknown>, search: URLSearchParams): Promise<Json> {
  switch (pathname) {
    case "/api/config":
...
async function signInWithPassword(body: Record<string, unknown>): Promise<Json> {
  ...
  const userId = asString((user as Record<string, unknown>).id, "user.id");
  applyConfig({
    supabaseUrl,
    supabaseAnonKey,
...
function assignEnv(name: string, value: unknown): void {
  if (typeof value === "string" && value.length > 0) {
    process.env[name] = value;
  }
}

function redactedConfig(): Json {
  return {
...
async function listSessions(search: URLSearchParams): Promise<Json> {
...
function requiredParam(search: URLSearchParams, name: string): string {
